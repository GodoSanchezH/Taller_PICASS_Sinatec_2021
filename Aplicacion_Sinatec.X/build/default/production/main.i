...
psect code
ConfigEUSART:
  ;Configuramos la ((PORTC) and 0FFh), 6, a de la UART
  BSF TRISC,7,c;((PORTC) and 0FFh), 7, a como entrada
  BCF TRISC,6,c;((PORTC) and 0FFh), 6, a como salida
  BCF TXSTA,6,c;((TXSTA) and 0FFh), 6, a; ((PORTC) and 0FFh), 6, a a 8 Bits
  BSF TXSTA,5,c;((TXSTA) and 0FFh), 5, a; ((PORTC) and 0FFh), 6, a habilitada
  BCF TXSTA,4,c;((TXSTA) and 0FFh), 4, a; UART ASÌNCRONO
  BSF TXSTA,2,c;((TXSTA) and 0FFh), 2, a; UART Alta velocidado
  ;Configuramos la recepción
  BSF RCSTA,7,c;((RCSTA) and 0FFh), 7, a;Puerto Serial habilitado
  BCF RCSTA,6,c;((RCSTA) and 0FFh), 6, a;Recepción a 8 Bits
  BSF RCSTA,4,c;((RCSTA) and 0FFh), 4, a;Recepción continua
  ;Configuramos la velocidad Baud rate
  BSF BAUDCON,3,c;((BAUDCON) and 0FFh), 3, a;UART trabajando a 16 Bits
  MOVLW 103
  MOVWF ((SPBRG) and 0FFh),c
  CLRF ((SPBRGH) and 0FFh),c;9600 Baudios para 4 Mhz

  RETURN

 psect code
SerialWrite:
  MOVWF ((TXREG) and 0FFh),C
espera:
  ;BANKSEL TXSTA
  BTFSS ((TXSTA) and 0FFh),1,c;((TXSTA) and 0FFh), 1, a
  goto espera
  return
...
PSECT Sinatec,global,class=CODE,RELOC=2
Sinatec:
    goto inicio
...
inicio:
    ...
    bsf ((OSCCON) and 0FFh),6,c ; ((OSCCON) and 0FFh), 6, a
    bsf ((OSCCON) and 0FFh),5,c ; ((OSCCON) and 0FFh), 5, a
    bcf ((OSCCON) and 0FFh),4,c ; ((OSCCON) and 0FFh), 4, a

    ;Configuracion de la GPIO
    ;TRISX -> 0: SALIDA <> 1: ENTRADA
    ;TRISDbits.TRD7 = 1
    MOVLW 00H
    MOVWF ((TRISD) and 0FFh),c
    MOVLW 255
    MOVWF ((LATD) and 0FFh),c
    call ConfigEUSART
    NOP
bucle:
; MOVLW 10101010B
